...
var associations = {
  // Pull the foreignKey into a convention
  apprentice: belongsTo("shop", {foreignKey: "apprenticeship_shop"}),
  shop: hasMany("apprentices", {foreignKey: "apprenticeship_shop"})
}
...
function buildResourceQuery(resource) {
  ...
  if (resource.id) {
    ...
    queryInput.loadAssociations = associations[queryInput.viewName]
  }
...
function belongsTo(resourceName, config) {
  return function(model, ready) {
    loadAssociation(model, resourceName, buildQuery({
      ...
      filter: {slug: model[config.foreignKey]},
...
function hasMany(resourceName, config) {
  return function(model, ready) {
    // Is there a more concise way to define a hash with a dynamic key?
    var filter = {}
    filter[config.foreignKey] = model.slug
